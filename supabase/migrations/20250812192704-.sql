...
-- Policies (SELECT only by org)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'tiny_v3_credentials' AND policyname = 'tiny_v3_credentials: org read'
  ) THEN
    CREATE POLICY "tiny_v3_credentials: org read" ON public.tiny_v3_credentials
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'tiny_v3_tokens' AND policyname = 'tiny_v3_tokens: org read'
  ) THEN
    CREATE POLICY "tiny_v3_tokens: org read" ON public.tiny_v3_tokens
    FOR SELECT USING (organization_id = public.get_current_org_id());
  END IF;
END $$;
